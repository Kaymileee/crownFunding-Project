...
const DashboardSideBar = () => {
  ...
  const navLinksClass =
    " flex items-center gap-x-5 md:w-12 md:h-12 md:justify-center md:rounded-xl md:mb-8 mb-4 text-iconColor  active:text-primary last:mt-auto last:bg-white last:shadow-sdPrimary ";
  return (
    <div className="w-full md:w-[76px] bg-white rounded-3xl shadow-[10px_10px_20px_rgba(218_213_213_0.15)] px-[14px] py-10 flex flex-col flex-shrink-0">
      {sideBarLinks.map((link) => (
        <NavLink
          to={link.url}
          key={link.title}
          className={` active:text-primary last:mt-auto last:bg-white last:shadow-sdPrimary active:bg-primary active:bg-opacity-20`}
        >
